...
> `npm run dev` поднимает Vite на `http://localhost:3000`, компилирует main-процесс в watch-режиме
> и автоматически открывает Electron. Для оконного режима используйте `npm run dev:window`.
...
│       │   └── [Screen]/
│       │       ├── [Screen].tsx
│       │       ├── [Screen].spec.ts      # Тесты рядом с компонентом
│       │       └── [Screen].spec.ts-snapshots/  # Скриншоты
│       ├── hooks/         # React хуки (включая useRouter)
│       └── tests/         # Утилиты для тестов
...
npm run start        # полноэкранный режим
npm run start:window # оконный режим
```
...
```bash
# ui-electron/.env.local (опционально)
